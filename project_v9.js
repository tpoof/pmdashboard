(function () {
  ...
  function buildOkrsSignature(selectedOkrFiscalYear) {
    return [
      "okrs",
      state.tasksVersion,
      state.projectsVersion,
      sigPart(selectedOkrFiscalYear),
      state.okrTableView,
    ].join("|");
  }

  function renderOkrsAnalytics(selectedOkrFiscalYear) {
    var sig = buildOkrsSignature(selectedOkrFiscalYear);
  ...
  function normalizeStatus(s) {
    return normalizePrimaryStatus(s);
  }

  function getKanbanColumnsOrdered() {
    var cols = getKanbanBaseColumns();
    var found = {};
    var hasUnknown = false;

    state.tasksAll.forEach(function (t) {
      if (isArchivedStatus(t.status)) return;
    ...
    if (hasUnknown && cols.indexOf("Unknown") === -1) cols.push("Unknown");
    return cols;
  }
  ...
  function setActiveTab(tabName) {
    ...
    renderProjectHealthSticky(
      tabName,
      tabName === "tasks" ? getSelected("pmProjectKeySelect") : "",
    );
  ...
  function buildTaskFilterState() {
    ...
    return {
      q: q,
      qCompact: qCompact,
      projectKey: getSelected("pmProjectKeySelect"),
      status: getSelected("pmStatusSelect"),
      assignee: getSelected("pmAssigneeSelect"),
      priority: getSelected("pmPrioritySelect"),
      category: getSelected("pmCategorySelect"),
  ...
  function taskMatchesFilters(task, filters) {
    ...
    if (
      filters &&
      filters.projectKey &&
      String(task.projectKey || "").trim() !== filters.projectKey
    )
      return false;
    if (
      filters &&
      filters.status &&
      normalizePrimaryStatus(task.status) !== filters.status
    )
      return false;
    if (
      filters &&
      filters.assignee &&
      String(task.assignedTo || "").trim() !== filters.assignee
    )
      return false;
    if (
      filters &&
      filters.priority &&
      String(task.priority || "").trim() !== filters.priority
    )
      return false;
    if (
      filters &&
      filters.category &&
      String(task.category || "").trim() !== filters.category
    )
      return false;
    return true;
  }
  ...
  function buildTaskFilterSignature(filters) {
    return [
      ...
      sigPart(filters.q),
      filters.devOnly ? "1" : "0",
      sigPart(filters.projectKey),
      sigPart(filters.status),
      sigPart(filters.assignee),
      sigPart(filters.priority),
      sigPart(filters.category),
    ].join("|");
  }
  ...
  function buildProjectFilterState() {
    ...
    return {
      q: q,
      qCompact: qCompact,
      fiscalYear: getSelected("pmProjectFiscalYearSelect"),
    };
  }
  ...
  function projectMatchesFilters(project, filters) {
    ...
    if (filters && filters.fiscalYear) {
      return (
        String(project.projectFiscalYear || "").trim() === filters.fiscalYear
      );
    }
    return true;
  }
  ...
  function renderTasksView(view, force) {
    ...
    renderProjectHealthSticky("tasks", filters.projectKey);
  ...
  function renderAnalyticsView(view) {
    if (view === "okrs") {
      ...
      var fy = getSelected("pmOkrFiscalYearSelect");
  ...
  function isInAnalyticsWindow(date, config) {
    ...
    if (config.year !== "all" && date.getFullYear() !== Number(config.year))
      return false;
    if (config.quarter && config.quarter !== "all") {
      var q = Math.floor(date.getMonth() / 3) + 1;
      if (config.quarter !== "Q" + q) return false;
    }
    return true;
  }
  ...
  function computeAnalyticsCache(analyticsTasks, analyticsProjects, config) {
    var now = new Date();
    var isAllYears = config.year === "all";
    var selectedYear = isAllYears ? null : Number(config.year);
    var selectedQuarter = config.quarter || "all";
    ...
    var filterLabel =
      (isAllYears ? "All years" : "Year " + selectedYear) +
  ...
  function renderAnalytics(tasks) {
    ...
    if (!cache) {
      cache = computeAnalyticsCache(analyticsTasks, analyticsProjects, {
        year: state.analyticsGeneralYear,
        quarter: state.analyticsGeneralQuarter,
      });
