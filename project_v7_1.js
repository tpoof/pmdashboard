(function () {
  ...
  var state = {
    ...
    kanbanColumns: [],
    filters: {
      projectFiscalYears: new Set(),
      projectKeys: new Set(),
      statuses: new Set(),
      assignees: new Set(),
      categories: new Set(),
      priorities: new Set(),
      analyticsYears: new Set(),
      analyticsQuarters: new Set(),
      okrFiscalYears: new Set(),
    },
    filterComponents: {},
    dataReady: false,
    sort: {
  ...
  var activeMultiSelect = null;

  function normalizeFilterValue(val) {
    return String(val == null ? "" : val).trim();
  }

  function filterSetHas(set, val) {
    if (!set || !set.size) return true;
    var v = normalizeFilterValue(val);
    if (!v) return false;
    return set.has(v);
  }
  ...
  function multiSelectDropdown(config) {
    ...
    var labelEl = config.labelEl || null;
    var labelId = config.labelId || (labelEl ? labelEl.id : "");
    if (labelEl && !labelEl.id) {
      labelEl.id = "pmLabel-" + Math.random().toString(36).slice(2, 8);
      labelId = labelEl.id;
    }
    var selectedSet = config.selectedSet || new Set();
    var options = [];
    var optionsMap = {};
    ...
    button.type = "button";
    button.className = "pm-multiSelectBtn";
    buttonText.className = "pm-multiSelectText";
    chevron.className = "pm-multiSelectChevron";
    ...
    panel.id = container.id ? container.id + "-panel" : "";
    panel.setAttribute("role", "dialog");
    panel.setAttribute(
      "aria-label",
      config.panelLabel || "Filter options",
    );
    panel.hidden = true;
    ...
    selectAllBtn.className = "pm-multiSelectAction";
    selectAllBtn.textContent = "Select all";
    clearBtn.type = "button";
    clearBtn.className = "pm-multiSelectAction";
    clearBtn.textContent = "Clear";
    actions.appendChild(selectAllBtn);
    actions.appendChild(clearBtn);

    searchRow.className = "pm-multiSelectSearchRow";
    searchInput.type = "text";
    searchInput.className = "pm-multiSelectSearch";
    ...
    function updateButtonLabel() {
      ...
      if (selectedSet.size === 1) {
        var onlyVal = Array.from(selectedSet)[0];
        buttonText.textContent = optionsMap[onlyVal] || onlyVal;
        return;
      }
      buttonText.textContent = selectedSet.size + " selected";
    }
    ...
    function renderOptions() {
      ...
      options.forEach(function (opt) {
        ...
        text.className = "pm-multiSelectOptionText";
        text.textContent = opt.label;
        label.appendChild(checkbox);
        label.appendChild(text);
        checkbox.addEventListener("change", function () {
          if (checkbox.checked) selectedSet.add(opt.value);
      ...
      searchRow.hidden = options.length <= 15;
      searchInput.value = "";
      updateButtonLabel();
    }

    function syncSelectionToOptions() {
      if (!selectedSet.size) return false;
      var optionValues = new Set(options.map(function (o) {
        return o.value;
      }));
      var removed = false;
      Array.from(selectedSet).forEach(function (val) {
    ...
    function setOptions(newOptions, opts) {
      options = (newOptions || [])
        .map(function (opt) {
          return {
            value: normalizeFilterValue(opt.value),
      ...
      var selectionChanged = syncSelectionToOptions();
      renderOptions();
      if (selectionChanged && !(opts && opts.suppressChange)) {
        onChange();
      }
    }
    ...
    function closePanel() {
      ...
      if (activeMultiSelect === api) activeMultiSelect = null;
    }

    function openPanel() {
      if (isOpen) return;
      if (activeMultiSelect && activeMultiSelect !== api) {
        activeMultiSelect.close();
      }
      activeMultiSelect = api;
      isOpen = true;
      panel.hidden = false;
    ...
    searchInput.addEventListener("input", function () {
      var q = normalizeForSearch(searchInput.value);
      Array.from(list.children).forEach(function (item) {
        var text = item.textContent || "";
        var match =
          !q || normalizeForSearch(text).indexOf(q) !== -1;
        item.hidden = !match;
      });
    });
    ...
    var api = {
      ...
      refreshLabel: updateButtonLabel,
      close: closePanel,
  ...
  function renderProjectHealthSticky(activeTab, selectedProjectKeys) {
    var wrap = document.getElementById("pmProjectHealthSticky");
    if (!wrap) return;

    var key = "";
    if (selectedProjectKeys && selectedProjectKeys.size === 1) {
      key = String(Array.from(selectedProjectKeys)[0] || "").trim();
    }
  ...
  function populateProjectKeyDropdown(projects) {
    var comp = state.filterComponents.projectKey;
    if (!comp) return;
    ...
    var options = cleaned.map(function (p) {
      var value = normalizeFilterValue(p.projectKey);
      var label =
    ...
    comp.setOptions(options, { suppressChange: true });
  }

  function populateProjectFiscalYearDropdown(projects) {
    var comp = state.filterComponents.projectFiscalYear;
    if (!comp) return;
    var vals = Array.from(
      new Set(
        (projects || [])
  ...
  function populateOkrFiscalYearDropdown(projects) {
    var comp = state.filterComponents.okrFiscalYear;
    if (!comp) return;
    var vals = Array.from(
      new Set(
        (projects || [])
  ...
  function populateAssigneeDropdown(tasks) {
    var comp = state.filterComponents.assignee;
    if (!comp) return;
    var vals = Array.from(
      new Set(
        tasks
  ...
  function populateCategoryDropdown(tasks) {
    var comp = state.filterComponents.category;
    if (!comp) return;
  ...
  function populateStatusDropdown(statuses) {
    var comp = state.filterComponents.status;
    if (!comp) return;
  ...
  function initFilterComponents() {
    state.filterComponents.projectFiscalYear = multiSelectDropdown({
      container: document.getElementById("pmProjectFiscalYearSelect"),
      labelEl: document.getElementById("pmProjectFiscalYearLabel"),
      selectedSet: state.filters.projectFiscalYears,
      panelLabel: "Project Fiscal Year filter",
      onChange: function () {
        applySearchAndFilters(true);
      },
    });
    state.filterComponents.projectKey = multiSelectDropdown({
      container: document.getElementById("pmProjectKeySelect"),
      labelEl: document.getElementById("pmProjectKeyLabel"),
      selectedSet: state.filters.projectKeys,
      panelLabel: "Project filter",
      onChange: function () {
        applySearchAndFilters(true);
      },
    });
    state.filterComponents.status = multiSelectDropdown({
      container: document.getElementById("pmStatusSelect"),
      labelEl: document.getElementById("pmStatusLabel"),
      selectedSet: state.filters.statuses,
      panelLabel: "Status filter",
      onChange: function () {
        applySearchAndFilters(true);
      },
    });
    state.filterComponents.assignee = multiSelectDropdown({
      container: document.getElementById("pmAssigneeSelect"),
      labelEl: document.getElementById("pmAssigneeLabel"),
      selectedSet: state.filters.assignees,
      panelLabel: "Assignee filter",
      onChange: function () {
        applySearchAndFilters(true);
      },
    });
    state.filterComponents.category = multiSelectDropdown({
      container: document.getElementById("pmCategorySelect"),
      labelEl: document.getElementById("pmCategoryLabel"),
      selectedSet: state.filters.categories,
      panelLabel: "Category filter",
      onChange: function () {
        applySearchAndFilters(true);
      },
    });
    state.filterComponents.priority = multiSelectDropdown({
      container: document.getElementById("pmPrioritySelect"),
      labelEl: document.getElementById("pmPriorityLabel"),
      selectedSet: state.filters.priorities,
      panelLabel: "Priority filter",
      onChange: function () {
        applySearchAndFilters(true);
      },
    });
    state.filterComponents.analyticsYear = multiSelectDropdown({
      container: document.getElementById("pmAnalyticsGeneralYearSelect"),
      labelEl: document.getElementById("pmAnalyticsGeneralYearLabel"),
      selectedSet: state.filters.analyticsYears,
      panelLabel: "Analytics year filter",
      onChange: function () {
        applySearchAndFilters(true);
      },
    });
    state.filterComponents.analyticsQuarter = multiSelectDropdown({
      container: document.getElementById("pmAnalyticsGeneralQuarterSelect"),
      labelEl: document.getElementById("pmAnalyticsGeneralQuarterLabel"),
      selectedSet: state.filters.analyticsQuarters,
      panelLabel: "Analytics quarter filter",
      onChange: function () {
        applySearchAndFilters(true);
      },
    });
    state.filterComponents.okrFiscalYear = multiSelectDropdown({
      container: document.getElementById("pmOkrFiscalYearSelect"),
      labelEl: document.getElementById("pmOkrFiscalYearLabel"),
      selectedSet: state.filters.okrFiscalYears,
      panelLabel: "OKR Fiscal Year filter",
      onChange: function () {
        applySearchAndFilters(true);
      },
    });

    if (state.filterComponents.priority) {
      state.filterComponents.priority.setOptions(
        [
  ...
  function applySearchAndFilters(renderOnlyCurrentTabFirst) {
    ...
    var selectedProjectKeys = state.filters.projectKeys;
    var selectedProjectFiscalYears = state.filters.projectFiscalYears;
    var selectedStatuses = state.filters.statuses;
    var selectedAssignees = state.filters.assignees;
    var selectedPriorities = state.filters.priorities;
    var selectedCategories = state.filters.categories;
    var selectedOkrFiscalYears = state.filters.okrFiscalYears;
    ...
    var okrFiltered = projectsFiltered.filter(function (p) {
      return filterSetHas(selectedOkrFiscalYears, p.okrFiscalYear);
    });

    var okrBaseProjects = state.projectsAll.filter(function (p) {
      return filterSetHas(selectedOkrFiscalYears, p.okrFiscalYear);
    });
    ...
    if (activeTab === "tasks") {
      tasksFiltered = tasksSearchFiltered.filter(function (t) {
        if (!filterSetHas(selectedProjectKeys, t.projectKey)) return false;
        if (!filterSetHas(selectedStatuses, t.status)) return false;
        if (!filterSetHas(selectedAssignees, t.assignedTo)) return false;
        if (!filterSetHas(selectedPriorities, t.priority)) return false;
        if (!filterSetHas(selectedCategories, t.category)) return false;
        return true;
      });
    }
    ...
    var projectsTableFiltered = projectsFiltered.filter(function (p) {
      return filterSetHas(selectedProjectFiscalYears, p.projectFiscalYear);
    });

    renderProjectHealthSticky(activeTab, selectedProjectKeys);
  ...
  function wireClearFilters() {
    function clearAll() {
      ...
      if (s) s.value = "";
      if (state.filterComponents.projectKey)
        state.filterComponents.projectKey.clearSelection();
      if (state.filterComponents.status)
        state.filterComponents.status.clearSelection();
      if (state.filterComponents.assignee)
        state.filterComponents.assignee.clearSelection();
      if (state.filterComponents.priority)
        state.filterComponents.priority.clearSelection();
      if (state.filterComponents.category)
        state.filterComponents.category.clearSelection();
      applySearchAndFilters(true);
    }
  ...
  function wireOkrFilters() {
    ...
    if (clearBtn)
      clearBtn.addEventListener("click", function () {
        if (state.filterComponents.okrFiscalYear)
          state.filterComponents.okrFiscalYear.clearSelection();
        applySearchAndFilters(true);
      });
  }
  ...
  function wireAnalyticsSharedFilters() {
    ...
    if (clearBtn)
      clearBtn.addEventListener("click", function () {
        if (state.filterComponents.analyticsYear)
          state.filterComponents.analyticsYear.clearSelection();
        if (state.filterComponents.analyticsQuarter)
          state.filterComponents.analyticsQuarter.clearSelection();
        applySearchAndFilters(true);
      });
  }
  ...
  function renderAnalytics(tasks) {
    ...
    if (state.filterComponents.analyticsYear) {
      state.filterComponents.analyticsYear.setOptions(
    ...
    var selectedYears = state.filters.analyticsYears || new Set();
    var selectedQuarters = state.filters.analyticsQuarters || new Set();
    var yearLabel = selectedYears.size
      ? "Years " +
        Array.from(selectedYears)
          ...
          .join(", ")
      : "All years";
    var quarterLabel = selectedQuarters.size
      ? "Quarters " +
        Array.from(selectedQuarters)
          .sort(function (a, b) {
            return a.localeCompare(b);
          })
          .join(", ")
    ...
    function inSelectedQuarter(d) {
      if (!d || isNaN(d.getTime())) return false;
      if (!selectedQuarters.size) return true;
      var q = Math.floor(d.getMonth() / 3) + 1;
      return selectedQuarters.has("Q" + q);
    }
    ...
    if (state.filterComponents.analyticsQuarter) {
      state.filterComponents.analyticsQuarter.setOptions(quarterOptions, {
        suppressChange: true,
      });
    }
  ...
  async function main() {
    try {
      ...
      var taskRows = taskRowsAll.filter(function (r) {
        return hasAnyS1Value(
          r,
          [8, 9, 10, 11, 12, 13, 14, 16, 17, 18, 30, 39],
        );
      });
      ...
      var searchEl = document.getElementById("pmSearchInput");
      if (searchEl) {
