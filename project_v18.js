(function () {
  ...
  // ── DEBUG: pmOverdueDebug() ──────────────────────────────────────────────────
  // Registered at module load — always available in the browser console.
  // Run pmOverdueDebug() to diagnose why the overdue alert may not be showing.
  // Remove this block once confirmed working in production.
  window.pmOverdueDebug = function () {
    ...
    console.group("3) Projects (ind 5 = owner, ind 70 = projectEndDate)");
    console.log("Total projects loaded:", (state.projectsAll || []).length);
    var projSample = (state.projectsAll || []).slice(0, 10).map(function (p) {
      return {
        recordID: p.recordID,
    ...
    console.log(
      "My overdue projects (" + myOverdueProjects.length + "):",
    ...
    console.group("4) Session dismiss");
    console.log("Key:", OVERDUE_ALERT_DISMISSED_KEY);
    console.log(
      "Dismissed this session?:",
      !!sessionStorage.getItem(OVERDUE_ALERT_DISMISSED_KEY),
    );
    console.log(
      '(Run sessionStorage.removeItem("' +
        OVERDUE_ALERT_DISMISSED_KEY +
        '") to reset)',
    );
    console.groupEnd();

    console.group("5) Alert element");
    var el = document.getElementById("pmOverdueAlert");
    console.log("Element found?:", !!el);
    if (el) console.log("Currently hidden?:", el.hidden);
    console.groupEnd();
