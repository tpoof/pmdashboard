...
<style>
  ...
  #pmAccessRequest .pm-ar-divider {
    border: none;
    border-top: 1px solid #e5e7eb;
    margin: 0 0 24px;
  }
  #pmAccessRequest .pm-ar-label {
    display: block;
    font-size: 0.8rem;
    font-weight: 600;
  ...
  #pmAccessRequest .pm-ar-userfield {
    width: 100%;
    box-sizing: border-box;
    border: 1px solid #d1d5db;
    border-radius: 6px;
  ...
  #pmAccessRequest .pm-ar-userfield .pm-ar-spinner {
    width: 14px;
    height: 14px;
    border: 2px solid #d1d5db;
    border-top-color: #6b7280;
    border-radius: 50%;
    animation: pm-ar-spin 0.7s linear infinite;
    flex-shrink: 0;
  }
  @keyframes pm-ar-spin {
    to { transform: rotate(360deg); }
  }
  #pmAccessRequest .pm-ar-submit {
    width: 100%;
    padding: 11px 20px;
    background: #1a4480;
    color: #fff;
  ...
  #pmAccessRequest .pm-ar-submit:hover {
    background: #162e51;
    color: #fff;
  }
  #pmAccessRequest .pm-ar-error {
    font-size: 0.88rem;
    color: #dc2626;
    margin: 10px 0 0;
    min-height: 20px;
  }
  #pmAccessRequest .pm-ar-success {
    text-align: center;
  }
...
<div id="pmAccessRequest">
  <div class="pm-ar-card">
    ...
    <div id="pmArForm">
      ...
      <p class="pm-ar-desc">
        This dashboard is limited to those who need to know.
        If this applies to you, submit a request below and your
        access will be reviewed.
      </p>
      <hr class="pm-ar-divider" />
      <label class="pm-ar-label" for="pmArUserDisplay">Requesting access as</label>
      <div class="pm-ar-userfield" id="pmArUserDisplay" aria-live="polite">
        <span class="pm-ar-spinner" id="pmArSpinner" aria-hidden="true"></span>
        <span id="pmArUserName">Resolving your account…</span>
      </div>
      <a
        id="pmArSubmitBtn"
        class="pm-ar-submit"
    ...
    <!-- Success state - not used in Option B but kept for reference -->
  </div>
</div>
...
<script>
(function () {
  var BASE = "/platform/projects";
  var env = document.getElementById("pmArEnv");
  var rawUserId   = (env && env.getAttribute("data-userid"))  || "";
  var displayName = (env && env.getAttribute("data-username")) || "";
  var smartyCsrf  = (env && env.getAttribute("data-csrf"))    || "";
  var nameEl    = document.getElementById("pmArUserName");
  var spinnerEl = document.getElementById("pmArSpinner");
  var submitBtn = document.getElementById("pmArSubmitBtn");
  var resolvedValue = rawUserId || displayName || "";

  if (displayName) { nameEl.textContent = displayName; }
  else if (rawUserId) { nameEl.textContent = rawUserId; }
  else { nameEl.textContent = "Current user"; }
  spinnerEl.style.display = "none";

  // ── DEBUG: surface CSRF and POST format on load ────────────────────────
  console.group("[PM Access Request] Environment Debug");
  console.log("userID (from Smarty):", rawUserId || "(empty)");
  console.log("displayName (from Smarty):", displayName || "(empty)");
  console.log("CSRFToken (from Smarty):", smartyCsrf ? "PRESENT (" + smartyCsrf.substring(0, 8) + "...)" : "EMPTY — will need API fetch");
  console.groupEnd();

  // ── DEBUG: Test CSRF API fetch ─────────────────────────────────────────
  fetch(BASE + "/api/form/new?csrfProbe=1", {
    credentials: "include",
    headers: { "x-requested-with": "XMLHttpRequest" }
  })
  .then(function(r) {
    console.log("[PM Access Request] CSRF probe status:", r.status, r.ok ? "OK" : "FAILED");
    return r.text();
  })
  .then(function(text) {
    console.log("[PM Access Request] CSRF probe response:", text.substring(0, 200));
    var token = "";
    try {
      var data = JSON.parse(text);
      token = data.CSRFToken || data.csrfToken || data.token || "";
    } catch(e) {}
    console.log("[PM Access Request] Parsed CSRF from API:", token ? "PRESENT (" + token.substring(0, 8) + "...)" : "NOT FOUND in response");
  })
  .catch(function(e) {
    console.log("[PM Access Request] CSRF probe error:", e.message);
  });

  // ── DEBUG: Test both POST formats for record creation ─────────────────
  // We test with a dry-run by checking what the API accepts (don't actually create)
  // First check if the endpoint is reachable at all
  fetch(BASE + "/api/form/new", {
    method: "GET",
    credentials: "include",
    headers: { "x-requested-with": "XMLHttpRequest" }
  })
  .then(function(r) {
    console.log("[PM Access Request] POST endpoint GET probe status:", r.status, r.ok ? "reachable" : "unreachable");
    return r.text();
  })
  .then(function(text) {
    console.log("[PM Access Request] POST endpoint response:", text.substring(0, 200));
  })
  .catch(function(e) {
    console.log("[PM Access Request] POST endpoint probe error:", e.message);
  });

  // ── Best-effort orgchart upgrade ───────────────────────────────────────
  var searchTerm = rawUserId || displayName;
  if (searchTerm) {
    fetch(BASE + "/api/orgchart?search=" + encodeURIComponent(searchTerm) + "&limit=5", {
      credentials: "include", headers: { "x-requested-with": "XMLHttpRequest" }
    })
    .then(function(r) {
      console.log("[PM Access Request] Orgchart probe status:", r.status, r.ok ? "OK" : "FAILED");
      return r.ok ? r.json() : null;
    })
    .then(function(data) {
      if (!data) return;
      var records = Array.isArray(data) ? data : Object.values(data);
      var match = records.find(function(rec) {
        return String(rec.empUID||"").toLowerCase() === String(rawUserId).toLowerCase()
          || String(rec.userName||"").toLowerCase() === String(rawUserId).toLowerCase();
      }) || records[0];
      if (!match) return;
      console.log("[PM Access Request] Orgchart match:", match);
      var parts = [match.firstName, match.lastName].filter(Boolean);
      var fullName = match.name || (parts.length ? parts.join(" ") : "");
      if (fullName) nameEl.textContent = fullName.trim();
      var better = match.userName || (match.empUID ? String(match.empUID) : "");
      if (better) resolvedValue = better;
    })
    .catch(function(e) {
      console.log("[PM Access Request] Orgchart error:", e.message);
    });
  }

  // ── Submit button — still uses native form URL for now ─────────────────
  function buildFormUrl() {
    var url = BASE + "/report.php?a=LEAF_Start_Request&id=form_42704";
    var fill = encodeURIComponent(resolvedValue || "");
    if (fill) url += "&104=" + fill;
    return url;
  }
  submitBtn.href = buildFormUrl();
})();
</script>
<!--{/if}-->