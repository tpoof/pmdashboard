<div id="pkWrap35" style="max-width: 820px;">
  <div id="pkSummary35" class="pm-picker-summary" style="display:flex; gap:0.75rem; align-items:center; flex-wrap:wrap;">
    <div class="pm-picker-summary-label" style="font-weight:600; min-width:120px;">OKR</div>
    <div id="pkSummaryVal35" class="pm-picker-summary-value" style="flex:1; min-width:200px; padding:0.25rem 0;">None selected</div>
    <button type="button" id="pkToggle35" class="buttonNorm pm-picker-change" aria-expanded="false" aria-controls="pkPanel35" style="white-space:nowrap;">Change</button>
  </div>

  <div id="pkPanel35" class="pm-picker-panel" style="display:none; margin-top:0.6rem;">
    <div class="pm-picker-search-row" style="display:flex; gap:0.75rem; align-items:flex-end; flex-wrap:wrap;">
      <div style="flex:1; min-width: 260px;">
        <input id="pkSearch35" class="pm-picker-search" type="text" autocomplete="off" placeholder="Search OKR key or name" aria-label="Search OKR key or name"
          style="width:100%; padding:0.45rem 0.55rem; border:1px solid #c9c9c9; border-radius:0.5rem;">
      </div>

      <div style="display:flex; gap:0.5rem; align-items:center;">
        <button type="button" id="pkSearchClear35" class="pm-picker-search-clear" hidden>Clear search</button>
        <button type="button" id="pkClear35" class="buttonNorm" style="white-space:nowrap;">Clear</button>
        <button type="button" id="pkClose35" class="buttonNorm" style="white-space:nowrap;">Close</button>
      </div>
    </div>

    <div id="pkMsg35" style="margin-top:0.5rem; font-size:0.9rem;"></div>

    <div style="margin-top:0.6rem;">
      <div style="font-weight:600; margin-bottom:0.35rem;">Available OKRs</div>
      <div id="pkList35" class="pm-picker-list"
...
<style>
  /* Hide only the real OKR-bound field for indicator 35 (keeps custom UI visible) */
  .response.blockIndicator_35 [name="35"] { display:none !important; }
  .response.blockIndicator_35 textarea[name="35"],
  .response.blockIndicator_35 input[name="35"],
  .response.blockIndicator_35 select[name="35"] { display:none !important; }
</style>
...
.pm-picker-empty { font-size:0.9rem; color:#5b6270; padding:0.4rem; }
  ...
  const wrap = document.getElementById("pkWrap35");
  const searchEl = document.getElementById("pkSearch35");
  const listEl = document.getElementById("pkList35");
  const summaryValEl = document.getElementById("pkSummaryVal35");
  const msgEl = document.getElementById("pkMsg35");
  const clearBtn = document.getElementById("pkClear35");
  const toggleBtn = document.getElementById("pkToggle35");
  const panelEl = document.getElementById("pkPanel35");
  const closeBtn = document.getElementById("pkClose35");
  const searchClearBtn = document.getElementById("pkSearchClear35");
  ...
  // Bind to the real OKR field for this indicator (35)
  ...
  function renderList() {
    ...
    listEl.innerHTML = visible.map(okr => {
      ...
      return `
        <label class="pm-picker-option ${checked ? "pm-picker-option--selected" : ""}" style="display:flex; gap:0.5rem; align-items:flex-start;">
          <input type="radio" name="pkRadio35" class="pkRadio35" data-key="${safe(okr.key)}" ${checked ? "checked" : ""} style="margin-top:0.2rem;">
    ...
    listEl.querySelectorAll(".pkRadio35").forEach(r => {
