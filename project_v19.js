(function () {
  ...
  function openModal(title, url, postLoadCallback, _skipHistory) {
    ...
    var isNewTaskModal =
      !_rMatch &&
      String(url || "").indexOf("form_9b302") !== -1;
    ...
    if (isNewTaskModal) {
      frame._newTaskStatusHandler = async function () {
        try {
          ...
          if (frameUrl.indexOf("printview") === -1 && frameUrl.indexOf("a=print") === -1) return;

          var newRecordID = newIdMatch[1];
          console.log(`[PM Dashboard] New task detected: recordID=${newRecordID} — writing Not Started`);
          ...
          if (resp.ok) {
            console.log(`[PM Dashboard] Not Started written successfully for task ${newRecordID}`);
          } else {
            console.warn(`[PM Dashboard] Not Started write failed for task ${newRecordID}: HTTP ${resp.status}`);
          }
        } catch (e) {
          console.warn("[PM Dashboard] Error writing Not Started for new task:", e);
        }
      };
  ...
  function toggleProjectDescriptionRow(btn) {
    ...
    var allToggles = parentRow.querySelectorAll(`.pm-projDescToggle[data-subrow="${subRowId}"]`);
    ...
    if (isExpanded) {
      ...
      allToggles.forEach(function(t) {
        ...
        t.querySelector(".pm-projDescChevron").classList.remove("pm-projDescChevron--open");
      });
    } else {
      ...
      allToggles.forEach(function(t) {
        ...
        t.querySelector(".pm-projDescChevron").classList.add("pm-projDescChevron--open");
      });
    }
  }
  ...
  function renderOkrsObjectivesTable(projects) {
    ...
    var rows = okrRows
      ...
      .map(function (p) {
        ...
        return `<tr><td>${okrLink || safe(p.okrKey)}</td><td>${safe(p.okrObjective)}</td><td>${safe(formatDateCell(p.okrStartDate))}</td><td>${safe(formatDateCell(p.okrEndDate))}</td><td>${safe(p.okrFiscalYear)}</td></tr>`;
      })
      .join("");
    ...
    el.innerHTML = `
      <table class="pm-table">
        ...
        <tbody>${rows || "<tr><td colspan='5'>No OKRs found</td></tr>"}</tbody>
      </table>`;
  ...
  function renderOkrsKeyResultsTable() {
    ...
    var rows = rowsData
      ...
      .map(function (row) {
        ...
        return `<tr><td>${okrLink || safe(row.okrKey)}</td><td>${row.keyResultsHtml}</td></tr>`;
      })
      .join("");
    ...
    el.innerHTML = `
      <table class="pm-table">
        ...
        <tbody>${rows || "<tr><td colspan='2'>No Key Results found</td></tr>"}</tbody>
      </table>`;
    ...
    if (!okrEntriesFiltered.length) {
      ...
      var cardsHtml = okrEntriesFiltered
        .map(function (entry, idx) {
              ...
              keyResultItems
                .map(function (kr) {
                    ...
                    ? "<ul class='pm-krProjectList'>" +
                      kr.projects
                        .map(function (p, projIdx) {
                          ...
                          var taskSummary = `Tasks: ${projectTasks.length} total, ${completedProjectTasks} completed`;
                          var projectTaskId = `${krId}-project-${projIdx}-tasks`;
                            ...
                            ? `<ul class='pm-krTaskList'>${projectTasks
                                .map(function (t) {
                                  ...
                                  var taskLink = taskHref
                                    ? `<a href="${safe(taskHref)}" class="pm-recordLink" data-title="${safe(`Task ${t.recordID}`)}">${safe(taskLabel)}</a>`
                                    : safe(taskLabel);
                                  return `<li class='pm-okrItem'>${taskLink} — ${safe(projectLabel)} (${statusLabel})</li>`;
                                })
                                .join("")}</ul>`
                            : "<div class='pm-okrItem'>No tasks for this project.</div>";

                          return `<li class='pm-krProject'>
                            <div class='pm-krProjectRow'>
                              <div class='pm-krProjectName'>${link}</div>
                              <div class='pm-krProjectMeta'>${safe(taskSummary)}</div>
                              <button type='button' class='pm-okrToggle pm-krProjectToggle' data-target='${projectTaskId}' data-label='Tasks' data-okr='${okrLabel}' data-count='${projectTasks.length}' aria-expanded='true' aria-controls='${projectTaskId}' aria-label='Hide Tasks for OKR ${okrLabel} key result ${safeAttr(kr.name)}'>Hide Tasks (${projectTasks.length})</button>
                            </div>
                            <div class='pm-okrDetails' id='${projectTaskId}' role='region' aria-label='Tasks for OKR ${okrLabel} key result ${safeAttr(kr.name)}' aria-hidden='false'>${taskList}</div>
                          </li>`;
                        })
                        .join("") +
                      "</ul>"
                    ...
                    ? `<ul class='pm-krOtherList'>${kr.otherTasks
                        .map(function (t) {
                          ...
                          var taskLink = taskHref
                            ? `<a href="${safe(taskHref)}" class="pm-recordLink" data-title="${safe(`Task ${t.recordID}`)}">${safe(taskLabel)}</a>`
                            : safe(taskLabel);
                          var projectMeta = `Project: ${getTaskProjectMeta(t.projectKey)}`;
                          return `<li class='pm-okrItem'>${taskLink} — <span class="pm-krOtherMeta">${safe(projectMeta)}</span> (${statusLabel})</li>`;
                        })
                        .join("")}</ul>`
                  ...
                  var exceptionBadge = hasExceptions
                    ? `<span class='pm-krBadge'>Exceptions: ${exceptionCount}</span>`
                    : "";
                  var countsLabel = `Tasks: ${kr.completedTasks} / ${kr.totalTasks} | Projects: ${kr.projects.length}`;
                  var otherToggleClass = `pm-okrToggle pm-krOtherToggle${hasExceptions ? "" : " pm-krOtherToggle--muted"}`;
                  var projectsToggle = `<button type='button' class='pm-okrToggle pm-krProjectsToggle' data-target='${projId}' data-label='Projects' data-okr='${okrLabel}' data-count='${kr.projects.length}' aria-expanded='true' aria-controls='${projId}' aria-label='Hide Projects for OKR ${okrLabel} key result ${safeAttr(kr.name)}'>Hide Projects (${kr.projects.length})</button>`;
                  var otherToggle = hasExceptions
                    ? `<button type='button' class='${otherToggleClass}' data-target='${otherId}' data-label='Other contributing items' data-okr='${okrLabel}' data-count='${exceptionCount}' aria-expanded='false' aria-controls='${otherId}' aria-label='Expand Other contributing items for OKR ${okrLabel} key result ${safeAttr(kr.name)}'>Other contributing items (${exceptionCount})</button>`
                    : "";
                  var otherBlock = hasExceptions
                    ? `<div class='pm-krDetailBlock'>${otherToggle}<div class='pm-okrDetails' id='${otherId}' role='region' aria-label='Other contributing items for OKR ${okrLabel} key result ${safeAttr(kr.name)}' aria-hidden='true' hidden><div class='pm-okrHelper'>Tasks tagged to this Key Result whose project is tagged to a different Key Result or is not set.</div>${otherList}</div></div>`
                    : "";

                  return `<li class='pm-krItem${extraClass}'>
                    <div class='pm-krRow${hasExceptions ? " pm-krRow--hasExceptions" : ""}'>
                      <div class='pm-krName${krHighlight}'>${safe(kr.name)}</div>
                      <div class='pm-krProgressWrap'>
                        <div class='pm-krProgress' aria-hidden='true'><div class='pm-krProgressBar' style='width:${kr.percent}%'></div></div>
                        <div class='pm-krPercent'>${kr.percent}%</div>
                      </div>
                      <div class='pm-krCounts'>${safe(countsLabel)}</div>
                      <div class='pm-krActions'>
                        ${exceptionBadge}
                        <button type='button' class='pm-krRowToggle' data-target='${krDetailsId}' aria-expanded='false' aria-controls='${krDetailsId}' data-kr='${safeAttr(kr.name)}' aria-label='Expand details for key result ${safeAttr(kr.name)}'>Details</button>
                      </div>
                    </div>
                    <div class='pm-okrDetails pm-krDetails' id='${krDetailsId}' role='region' aria-label='Details for key result ${safeAttr(kr.name)}' aria-hidden='true' hidden>
                      <div class='pm-krDetailBlock'>
                        ${projectsToggle}
                        <div class='pm-okrDetails' id='${projId}' role='region' aria-label='Projects for OKR ${okrLabel} key result ${safeAttr(kr.name)}' aria-hidden='false'>${projList}</div>
                      </div>
                      ${otherBlock}
                    </div>
                  </li>`;
          ...
          return `<div class='pm-okrCard${idx >= visibleCount ? " is-hidden" : ""}${cardExpanded ? " is-expanded" : ""}' id='${cardId}' data-okr-index='${idx}' data-okr-key='${okrLabel}'>
            <div class='pm-okrCardHeader'>
              <div class='pm-okrTitle'>${safe(objective.title || "Untitled objective")}</div>
              <div class='pm-okrMeta'>
                <span class='pm-okrKeyBadge'>${okrKeyLink || safe(okrKey)}</span>
                <span class='${pctClass}'>${avgPercent}%</span>
              </div>
            </div>
            <div class='pm-okrProgress' role='img' aria-label='Objective ${okrLabel} is ${avgPercent}% complete'>
              <div class='pm-okrProgressBar' style='width:${avgPercent}%'></div>
            </div>
            <div class='pm-okrSummaryRow'>
              <div class='pm-okrStats'>
                <span class='pm-okrStat'>Key Results: ${keyResultItems.length}</span>
                <span class='pm-okrStat'>Projects: ${okrProjectCount}</span>
                <span class='pm-okrStat'>Tasks: ${okrTaskCount}</span>
              </div>
              <button type='button' class='pm-okrCardToggle' data-target='${cardBodyId}' aria-expanded='${cardExpanded ? "true" : "false"}' aria-controls='${cardBodyId}' data-okr='${okrLabel}' aria-label='${cardExpanded ? "Collapse OKR " : "Expand OKR "}${okrLabel}'>${cardExpanded ? "Collapse" : "Expand"}</button>
            </div>
            <div class='pm-okrCardBody' id='${cardBodyId}' role='region' aria-label='Key Results for OKR ${okrLabel}' aria-hidden='${cardExpanded ? "false" : "true"}'${cardExpanded ? "" : " hidden"}>
              <div class='pm-krSection'>
                <div class='pm-krHeader'><div class='pm-krHeaderTitle'>Key Results (${keyResultItems.length})</div></div>
                ${keyResultList}
              </div>
            </div>
          </div>`;
      ...
      var quickOverallPercent = okrEntriesFiltered.length
        ...
        : 0;
      if (quickStatsEl) {
        quickStatsEl.innerHTML = `<span class='pm-okrQuickStat'>Objectives: ${okrEntriesFiltered.length}</span><span class='pm-okrQuickStat'>Key Results: ${quickKrCount}</span><span class='pm-okrQuickStat'>Overall: ${quickOverallPercent}%</span>`;
      }

      if (indexWrap) {
        var indexHtml = `<div class='pm-okrIndexGrid'>${indexItems
          .map(function (item) {
            var title = item.title || "Untitled objective";
            return `<div class='pm-okrIndexItem' role='button' tabindex='0' data-target='${item.cardId}' data-okr='${safeAttr(item.okrKey)}' aria-label='Jump to OKR ${safeAttr(item.okrKey)}'>
              <div class='pm-okrIndexKey'>${safe(item.okrKey)}</div>
              <div class='pm-okrIndexTitle' title='${safeAttr(title)}'>${safe(title)}</div>
              <div class='pm-okrIndexProgressWrap'>
                <div class='pm-okrIndexProgress' aria-hidden='true'><div class='pm-okrIndexBar' style='width:${item.percent}%'></div></div>
                <div class='pm-okrIndexPercent'>${item.percent}%</div>
              </div>
            </div>`;
          })
          .join("")}</div>`;
  ...
  function updateCellDisplay(cell, indicatorId, formType, value) {
    ...
    if (formType === "task" && indNum === TASK_IND.status) {
      ...
      // Re-inject workflow icon (outside inline editor scope — icon is pre-pended, not part of editor)
      ...
      if (rid && state.stepIDCache[rid] !== undefined) {
        ...
        if (iconHtml) {
          ...
          if (tmp.firstElementChild) cell.insertBefore(tmp.firstElementChild, cell.firstChild);
        }
      }
      return;
    }
    if (formType === "project" && indNum === PROJECT_IND.projectStatus) {
      ...
      if (prid && state.stepIDCache[prid] !== undefined) {
        ...
        if (piconHtml) {
          ...
          if (ptmp.firstElementChild) cell.insertBefore(ptmp.firstElementChild, cell.firstChild);
        }
      }
      return;
    }
  ...
  function updateStateAfterEdit(recordId, indicatorId, formType, value) {
    ...
    if (formType === "task") {
      ...
      for (var j = 0; j < (state.projectsAll || []).length; j++) {
        ...
        else if (indNum === PROJECT_IND.projectStartDate) p.projectStartDate = value;
        else if (indNum === PROJECT_IND.projectEndDate) p.projectEndDate = value;
        break;
      }
    }
  }
  ...
  function injectWorkflowIcons(containerEl) {
    ...
    var cells = containerEl.querySelectorAll("td.pm-editableCell[data-ind][data-record-id]");
    cells.forEach(function(cell) {
      ...
      var existing = cell.querySelector(".pm-workflowAlert");
      if (existing) existing.remove();
      // Get current status value from the cell
      var currentValue = cell.getAttribute("data-current-value") || cell.textContent.trim();
  ...
  async function fetchAndInjectWorkflowIcons(containerEl) {
    ...
    uncachedIds.forEach(function(id) { _stepIDFetchPending.add(id); });
    ...
    try {
      ...
      if (!_stepIDDebugLogged) {
        ...
        console.group("[PM Dashboard] stepID debug — confirm STEP_ID_RESOLVED value");
        console.log("recordID → stepID → dashboard status:");
        rows.forEach(function(row) {
          var rid = String(row.recordID || row.recordId || "").trim();
          var stepId = state.stepIDCache[rid] ? state.stepIDCache[rid].stepID : "?";
          // Find the status for this record
          var task = state.tasksById && state.tasksById.get(rid);
          var proj = !task && (state.projectsAll || []).find(function(p) { return String(p.recordID) === rid; });
          var status = task ? task.status : (proj ? proj.projectStatus : "?");
          console.log(`  recordID=${rid}  stepID=${stepId}  status="${status}"`);
        });
        console.log("Once confirmed, update STEP_ID_RESOLVED constant (currently:", STEP_ID_RESOLVED, ")");
        console.groupEnd();
      }
  ...
  function drilldownTicketsImported(label, cache) {
    ...
    var projectRows = importedProjects.map(function(p) {
      ...
      var pkCell = pkHref
        ...
        : safe(p.projectKey);
      return `<tr><td>${pkCell}</td><td>${safe(p.projectName || "(No name)")}</td><td>—</td><td>${safe(p.projectKey)}</td><td>${safe(p.owner)}</td><td>Project</td></tr>`;
    });
  ...
  function drilldownProjectsByType(label, cache) {
    ...
    var rows = projects.map(function (p) {
      var pkHref = getProjectRecordHrefFromKey(p.projectKey);
      var pkCell = pkHref
        ? `<td><a href="${safeAttr(pkHref)}" class="pm-recordLink pm-pkProjectLink" data-title="Project ${safeAttr(p.projectKey)}" data-projectkey="${safeAttr(p.projectKey)}">${safe(p.projectKey)}</a></td>`
        : `<td>${safe(p.projectKey)}</td>`;
      return `<tr>${pkCell}<td>${safe(p.projectName)}</td><td>${safe(p.projectStatus)}</td><td>${safe(p.owner)}</td><td>${safe(p.projectFiscalYear)}</td></tr>`;
    });
  ...
  function renderAnalyticsTable(
    ...
    var rangeText = `Showing ${shown} of ${total} result${total === 1 ? "" : "s"}`;
  ...
  function renderAnalyticsTablesFromCache(cache) {
    ...
    tableState.overdueRows = overdueTasks.map(function (t) {
      var pkHref = getProjectRecordHrefFromKey(t.projectKey);
      var pkLink = pkHref
        ? `<a href='${safe(pkHref)}' class='pm-recordLink pm-pkProjectLink' data-title='Project ${safe(t.projectKey)}' data-projectkey='${safeAttr(String(t.projectKey || ""))}'>${safe(t.projectKey)}</a>`
        : safe(t.projectKey);

      var taskHref = t.href || "";
      var taskLink = taskHref
        ? `<a href='${safe(taskHref)}' class='pm-recordLink' data-title='Task ${safe(t.recordID)}'>${safe(t.recordID)}</a>`
        : safe(t.recordID);

      return `<tr><td>${pkLink}</td><td>${taskLink}</td><td>${safe(t.title)}</td><td>${safe(t.assignedTo)}</td><td class='pm-overdueRed'>${safe(t.due)} (Overdue)</td><td>${renderStatusCell(t)}</td></tr>`;
    });
  ...
  async function main() {
    try {
      ...
      // ── Progress UI helpers ────────────────────────────────────────────────
      // We show a subtle loading message in the page <title> while data loads,
