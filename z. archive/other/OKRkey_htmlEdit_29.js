<div id="pkWrap29" style="max-width: 820px;">
  <div id="pkSummary29" class="pm-picker-summary" style="display:flex; gap:0.75rem; align-items:center; flex-wrap:wrap;">
    <div class="pm-picker-summary-label" style="font-weight:600; min-width:120px;">OKR</div>
    <div id="pkSummaryVal29" class="pm-picker-summary-value" style="flex:1; min-width:200px; padding:0.25rem 0;">None selected</div>
    <button type="button" id="pkToggle29" class="buttonNorm pm-picker-change" aria-expanded="false" aria-controls="pkPanel29" style="white-space:nowrap;">Change</button>
  </div>

  <div id="pkPanel29" class="pm-picker-panel" style="display:none; margin-top:0.6rem;">
    <div class="pm-picker-search-row" style="display:flex; gap:0.75rem; align-items:flex-end; flex-wrap:wrap;">
      <div style="flex:1; min-width: 260px;">
        <input id="pkSearch29" class="pm-picker-search" type="text" autocomplete="off" placeholder="Search OKR key or name" aria-label="Search OKR key or name"
          style="width:100%; padding:0.45rem 0.55rem; border:1px solid #c9c9c9; border-radius:0.5rem;">
      </div>

      <div style="display:flex; gap:0.5rem; align-items:center;">
        <button type="button" id="pkSearchClear29" class="pm-picker-search-clear" hidden>Clear search</button>
        <button type="button" id="pkClear29" class="buttonNorm" style="white-space:nowrap;">Clear</button>
        <button type="button" id="pkClose29" class="buttonNorm" style="white-space:nowrap;">Close</button>
      </div>
    </div>
    ...
    <div style="margin-top:0.6rem;">
      ...
      <div id="pkList29" class="pm-picker-list"
...
.pm-picker-empty { font-size:0.9rem; color:#5b6270; padding:0.4rem; }
  ...
  const summaryValEl = document.getElementById("pkSummaryVal29");
  const msgEl = document.getElementById("pkMsg29");
  const clearBtn = document.getElementById("pkClear29");
  const toggleBtn = document.getElementById("pkToggle29");
  const panelEl = document.getElementById("pkPanel29");
  const closeBtn = document.getElementById("pkClose29");
  const searchClearBtn = document.getElementById("pkSearchClear29");
