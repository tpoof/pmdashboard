(function () {
  ...
  var STORAGE_KEYS = {
    ...
    tasksDevOnly: "pmdashboard_tasks_devOnly_v8",
  };
  ...
  async function handleKanbanStatusChange(taskId, newStatus) {
    ...
    var task = state.tasksAll[idx];
    var oldStatus = task.status;
    var oldOther = task.otherSubType;
    var targetStatus = normalized;
    ...
    if (normalized === "Other") {
      ...
      task.otherSubType = "";
      applySearchAndFilters(true);
      announceKanbanStatus("Moved task " + taskId + " to " + targetStatus + ".");
    }
  ...
  function applySearchAndFilters(renderOnlyCurrentTabFirst) {
    ...
    if (state.devOnly) {
      tasksFiltered = tasksFiltered.filter(isDevelopmentTask);
    }
    if (activeTab === "tasks") {
      tasksFiltered = tasksFiltered.filter(function (t) {
        if (
          selectedProjectKey &&
        ...
        if (
          selectedStatus &&
          normalizePrimaryStatus(t.status) !== selectedStatus
        )
          return false;
        if (
          selectedAssignee &&
    ...
    var tasksSearchNoArchive = state.tasksAll.filter(function (t) {
      return !isArchivedStatus(t.status);
    });
  ...
  function renderAnalytics(tasks) {
    ...
    if (unknownCount > 0) statusLabels.push("Unknown");
    var statusData = statusLabels.map(function (label) {
      if (label === "Other (Blocked)") return otherBuckets["Other (Blocked)"];
      if (label === "Other (On Hold)") return otherBuckets["Other (On Hold)"];
      if (label === "Unknown") return unknownCount;
      return statusCounts[label] || 0;
    });
    setChartSummary(
      "pmChartTasksByStatusDesc",
      "Tasks by status (" +
